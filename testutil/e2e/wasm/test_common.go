...
import (
	...
	// "github.com/cosmos/ibc-go/v6/v6/modules/apps/transfer"
...
func createTestInput(
	...
	bh := bankkeeper.NewMsgServerImpl(bankKeeper)
	if br, err := bh.Send(ctx, &banktypes.MsgSend{
		Amount:      sdk.NewCoins(sdk.NewCoin("uollo", sdk.NewInt(1000000))),
		FromAddress: string(ctx.BlockHeader().ProposerAddress),
		ToAddress:   liqtypes.DefaultDustCollectorAddress.String(),
	}); err != nil {
		fmt.Println(br)
	}
	bh.Send(ctx, &banktypes.MsgSend{})
	router.AddRoute(sdk.NewRoute(banktypes.RouterKey, func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		res, err := bh.Send(ctx, msg.(*banktypes.MsgSend))
		return sdk.WrapServiceResult(ctx, res, err)
	}))
	router.AddRoute(sdk.NewRoute(banktypes.RouterKey, func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		res, err := bh.MultiSend(ctx, &banktypes.MsgMultiSend{
			Outputs: []banktypes.Output{
				banktypes.NewOutput(ctx.BlockHeader().ProposerAddress, sdk.NewCoins(sdk.NewCoin("uollo", sdk.NewInt(1000000)))),
			},
			Inputs: []banktypes.Input{
				banktypes.NewInput(ctx.BlockHeader().ProposerAddress, sdk.NewCoins(sdk.NewCoin("uollo", sdk.NewInt(1000000)))),
				banktypes.NewInput(ctx.BlockHeader().ProposerAddress, sdk.NewCoins(sdk.NewCoin("uollo", sdk.NewInt(1000000)))),
			},
		})
		return sdk.WrapServiceResult(ctx, res, err)
	...
	router.AddRoute(sdk.NewRoute(stakingtypes.RouterKey, func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		res, err := sh.Delegate(ctx, &stakingtypes.MsgDelegate{
			DelegatorAddress: string(ctx.BlockHeader().ProposerAddress),
			Amount:           sdk.NewCoin("uollo", sdk.NewInt(1000000)),
			ValidatorAddress: liqtypes.DefaultDustCollectorAddress.String(),
		})
		return sdk.WrapServiceResult(ctx, res, err)
	}))
	router.AddRoute(sdk.NewRoute(stakingtypes.RouterKey, func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		res, err := sh.Undelegate(ctx, &stakingtypes.MsgUndelegate{
			DelegatorAddress: string(ctx.BlockHeader().ProposerAddress),
			Amount:           sdk.NewCoin("uollo", sdk.NewInt(1000000)),
			ValidatorAddress: liqtypes.DefaultDustCollectorAddress.String(),
		})
	...
	dh := distributionkeeper.NewMsgServerImpl(distKeeper)
	router.AddRoute(sdk.NewRoute(distributiontypes.RouterKey, func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		res, err := dh.FundCommunityPool(ctx, &distributiontypes.MsgFundCommunityPool{
			Amount:    sdk.NewCoins(sdk.NewCoin("uollo", sdk.NewInt(1000000))),
			Depositor: string(ctx.BlockHeader().ProposerAddress),
		})
		return sdk.WrapServiceResult(ctx, res, err)
	}))
