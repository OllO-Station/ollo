...
// CmdRequestBandPricesData creates and broadcast a BandPrices request transaction
func CmdRequestBandPricesData() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "band-prices-data [oracle-script-id] [requested-validator-count] [sufficient-validator-count]",
		Short: "Make a new BandPrices query request via an existing BandChain oracle script",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			calldata := &types.BandPricesCallData{
				Symbols:    symbols,
			...
			msg := types.NewMsgBandPricesData(
