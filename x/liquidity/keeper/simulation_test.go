...
func (s *KeeperTestSuite) TestSimulation1() {
	...
	fuzz := func() {
		for i := 0; i < numBlocks; i++ {
			...
			for _, pool := range pools {
				totalBalancesBefore = totalBalancesBefore.Add(
					s.getBalances(pool.GetReserveAddress())...)
				totalBalancesBefore = totalBalancesBefore.Sub(
					sdk.NewCoins(
						sdk.NewCoin(
							pool.PoolCoinDenom,
							totalBalancesBefore.AmountOf(pool.PoolCoinDenom),
						),
					)[0],
				)
			}
			...
			for j := 0; j < numOrders; j++ {
				...
				if r.Intn(2) == 0 { // 50% chance
					...
					amt := utils.RandomInt(
						r,
						sdk.NewInt(1000),
						sdk.NewDecFromInt(s.getBalance(orderer, "denom2").
							Amount).
							QuoTruncate(price).
							TruncateInt(),
					)
			...
			for j := 0; j < numDeposits; j++ {
				...
				msg := types.NewMsgDeposit(
					depositor,
					pool.Id,
					simtypes.RandSubsetCoins(r, balances),
				)
			...
			for j := 0; j < numWithdraws; j++ {
				...
				msg := types.NewMsgWithdraw(
					withdrawer,
					pool.Id,
					sdk.NewCoin(pool.PoolCoinDenom, utils.RandomInt(r, sdk.NewInt(1), balance)),
				)
			...
			for _, pool := range pools {
				totalBalancesAfter = totalBalancesAfter.Add(
					s.getBalances(pool.GetReserveAddress())...)
				totalBalancesAfter = totalBalancesAfter.Sub(
					sdk.NewCoins(
						sdk.NewCoin(
							pool.PoolCoinDenom,
							totalBalancesAfter.AmountOf(pool.PoolCoinDenom),
						),
					)[0],
				)
			}
	...
	s.createPool(
		s.addr(10000),
		pair.Id,
		utils.ParseCoins("1000000000denom1,1000000000denom2"),
		true,
	)
	fuzz()
	...
	s.createRangedPool(
		...
		sdk.NewDec(2).Sub(sdk.NewDecWithPrec(1, 4)),
		utils.ParseDec("1.0"),
		true,
	)
	fuzz()
	...
	s.createRangedPool(
		...
		sdk.NewDecFromInt(sdkmath.NewIntWithDecimal(1, 20)),
		utils.ParseDec("1.0"),
		true,
	)
	fuzz()
