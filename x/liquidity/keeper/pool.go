	...
	for _, coin := range msg.DepositCoins {
		if coin.Denom != pair.BaseCoinDenom && coin.Denom != pair.QuoteCoinDenom {
			return sdkerrors.Wrapf(
				types.ErrInvalidCoinDenom,
				"coin denom %s is not in the pair",
				coin.Denom,
			)
		}
...
func (k Keeper) CreatePool(ctx sdk.Context, msg *types.MsgCreatePool) (types.Pool, error) {
	...
	x, y := msg.DepositCoins.AmountOf(
		pair.QuoteCoinDenom,
	), msg.DepositCoins.AmountOf(
		pair.BaseCoinDenom,
	)
...
func (k Keeper) ValidateMsgCreateRangedPool(ctx sdk.Context, msg *types.MsgCreateRangedPool) error {
	...
	if msg.MinPrice.LT(lowestTick) {
		return sdkerrors.Wrapf(
			...
			"min price must not be less than %s",
			lowestTick,
		)
	}
...
func (k Keeper) CreateRangedPool(
	ctx sdk.Context,
...
func (k Keeper) ValidateMsgDeposit(ctx sdk.Context, msg *types.MsgDeposit) error {
	...
	if pool.Type == types.PoolTypeBasic && len(msg.DepositCoins) != 2 {
		return sdkerrors.Wrapf(
			sdkerrors.ErrInvalidRequest,
			"wrong number of deposit coins: %d",
			len(msg.DepositCoins),
		)
	}
...
func (k Keeper) ExecuteDepositRequest(ctx sdk.Context, req types.DepositRequest) error {
	...
	ax, ay, pc := amm.Deposit(
		rx.Amount,
		ry.Amount,
		ps,
		req.DepositCoins.AmountOf(pair.QuoteCoinDenom),
		req.DepositCoins.AmountOf(pair.BaseCoinDenom),
	)
	...
	acceptedCoins := sdk.NewCoins(
		sdk.NewCoin(pair.QuoteCoinDenom, ax),
		sdk.NewCoin(pair.BaseCoinDenom, ay),
	)
...
func (k Keeper) FinishDepositRequest(
	ctx sdk.Context,
...
func (k Keeper) FinishWithdrawRequest(
	ctx sdk.Context,
