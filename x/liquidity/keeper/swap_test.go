...
func (s *KeeperTestSuite) TestMatchWithLowPricePool() {
	...
	s.createPool(
		s.addr(0),
		pair.Id,
		utils.ParseCoins("100000000000000000denom1,1000000denom2"),
		true,
	)
	order := s.buyLimitOrder(
		s.addr(1),
		pair.Id,
		utils.ParseDec("0.000000000000010000"),
...
func (s *KeeperTestSuite) TestDustCollector() {
	...
	s.Require().
		True(coinsEq(utils.ParseCoins("1denom2"), s.getBalances(s.keeper.GetDustCollector(s.ctx))))
}
...
func (s *KeeperTestSuite) TestFitPrice() {
	...
	for _, tc := range []struct {
		...
		s.Run(tc.name, func() {
			...
			switch tc.dir {
			case types.OrderDirectionBuy:
				offerCoin = sdk.NewCoin(
					pair.QuoteCoinDenom,
					tc.price.MulInt(amt).Ceil().TruncateInt(),
				)
...
func (s *KeeperTestSuite) TestRejectSmallOrders() {
	...
	s.Require().
		EqualError(msg.ValidateBasic(), "offer coin 99denom2 is smaller than the min amount 100: invalid request")
	...
	s.Require().
		EqualError(msg.ValidateBasic(), "order amount 99 is smaller than the min amount 100: invalid request")
	...
	s.Require().
		EqualError(msg2.ValidateBasic(), "offer coin 99denom1 is smaller than the min amount 100: invalid request")
	...
	msg2 = types.NewMsgMarketOrder(
		...
		"denom2", sdk.NewInt(99), 0)
	s.Require().
		EqualError(msg2.ValidateBasic(), "order amount 99 is smaller than the min amount 100: invalid request")
...
func (s *KeeperTestSuite) TestPoolOrderOverflow() {
	...
	s.createPool(
		s.addr(0),
		pair.Id,
		sdk.NewCoins(sdk.NewInt64Coin("denom1", 1e6), sdk.NewCoin("denom2", i)),
		true,
	)
...
func (s *KeeperTestSuite) TestSwap_edgecase2() {
	...
	s.createPool(
		s.addr(0),
		pair.Id,
		utils.ParseCoins("1005184935980denom2,601040339855denom1"),
		true,
	)
	s.createRangedPool(
...
func (s *KeeperTestSuite) TestSwap_edgecase3() {
	...
	s.createPool(
		s.addr(0),
		pair.Id,
		utils.ParseCoins("110001546090denom2,110013588106denom1"),
		true,
	)
	s.createRangedPool(
...
func (s *KeeperTestSuite) TestOrderBooks_edgecase1() {
	...
	s.createPool(
		s.addr(0),
		pair.Id,
		utils.ParseCoins("991883358661denom2,620800303846denom1"),
		true,
	)
	s.createRangedPool(
	...
	s.Require().
		True(intEq(sdk.NewInt(1178846737645), resp.Pairs[0].OrderBooks[0].Buys[0].UserOrderAmount))
	s.Require().True(decEq(utils.ParseDec("0.5187"), resp.Pairs[0].OrderBooks[0].Buys[1].Price))
	s.Require().
		True(intEq(sdk.NewInt(13340086), resp.Pairs[0].OrderBooks[0].Buys[1].UserOrderAmount))
...
func (s *KeeperTestSuite) TestPoolPreserveK() {
	...
	for _, pool := range pools {
		...
		ks[pool.Id] = sdk.NewDecFromInt(rx.Amount).
			Add(transX).
			Mul(sdk.NewDecFromInt(ry.Amount).Add(transY))
	}
	...
	for i := 0; i < 20; i++ {
		...
		for _, pool := range pools {
			...
			ammPool := pool.AMMPool(rx.Amount, ry.Amount, sdk.Int{}).(*amm.RangedPool)
			transX, transY := ammPool.Translation()
			k := sdk.NewDecFromInt(rx.Amount).
				Add(transX).
				Mul(sdk.NewDecFromInt(ry.Amount).Add(transY))
			s.Require().
				True(k.GTE(ks[pool.Id].Mul(utils.ParseDec("0.99999"))))
