...
// getNextDepositRequestIdWithUpdate increments the pool's Prev deposit request
// id and returns it.
func (k Keeper) getNextDepositRequestIdWithUpdate(ctx sdk.Context, pool types.Pool) uint64 {
	id := pool.PrevDepositRequestId + 1
	pool.PrevDepositRequestId = id
	k.SetPool(ctx, pool)
	return id
}

// getNextWithdrawRequestIdWithUpdate increments the pool's Prev withdraw
// request id and returns it.
func (k Keeper) getNextWithdrawRequestIdWithUpdate(ctx sdk.Context, pool types.Pool) uint64 {
	id := pool.PrevWithdrawRequestId + 1
	pool.PrevWithdrawRequestId = id
	k.SetPool(ctx, pool)
	return id
}
...
// MarkPoolAsInactive marks a pool as Inactive.
func (k Keeper) MarkPoolAsInactive(ctx sdk.Context, pool types.Pool) {
	pool.Inactive = true
	k.SetPool(ctx, pool)
}
...
func (k Keeper) ValidateMsgCreatePool(ctx sdk.Context, msg *types.MsgCreatePool) error {
	...
	_ = k.IteratePoolsByPair(ctx, pair.Id, func(pool types.Pool) (stop bool, err error) {
		if pool.TypeId == 1 && !pool.Inactive {
			duplicate = true
			return true, nil
		}
		if !pool.Inactive {
...
func (k Keeper) CreatePool(ctx sdk.Context, msg *types.MsgCreatePool) (types.Pool, error) {
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributeKeyReserveAddress, pool.ReserveAccountAddress),
...
func (k Keeper) ValidateMsgCreateIntelligentPool(ctx sdk.Context, msg *types.MsgCreateIntelligentPool) error {
...
func (k Keeper) CreateIntelligentPool(ctx sdk.Context, msg *types.MsgCreateIntelligentPool) (types.Pool, error) {
	if err := k.ValidateMsgCreateIntelligentPool(ctx, msg); err != nil {
		return types.Pool{}, err
	}
	...
	x, y := msg.DepositCoins.AmountOf(pair.QuoteCoinDenom), msg.DepositCoins.AmountOf(pair.BaseCoinDenom)
	ammPool, err := amm.CreateIntelligentPool(x, y, msg.MinPrice, msg.MaxPrice, msg.InitialPrice)
	if err != nil {
		return types.Pool{}, sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, err.Error())
	}
