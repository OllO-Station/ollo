	...
	return func(
		...
		if !found {
			return simtypes.NoOpMsg(
				...
				"all pairs have already been created",
			), nil, nil
		}
		...
		depositCoins := sdk.NewCoins(
			sdk.NewCoin(
				...
				utils.RandomInt(
					r,
					minDepositAmt,
					spendable.Sub(params.PoolCreationFee...).AmountOf(pair.BaseCoinDenom),
				),
			),
			sdk.NewCoin(
				pair.QuoteCoinDenom,
				utils.RandomInt(
					r,
					minDepositAmt,
					spendable.Sub(params.PoolCreationFee...).AmountOf(pair.QuoteCoinDenom),
				),
			),
		)
...
func SimulateMsgCreateRangedPool(
	ak types.AccountKeeper,
	bk types.BankKeeper,
	k keeper.Keeper,
) simtypes.Operation {
	return func(
		...
		for {
			...
			minPrice = amm.RandomTick(
				r,
				utils.ParseDec("0.00001"),
				utils.ParseDec("1"),
				int(tickPrec),
			)
			maxPrice = amm.RandomTick(
				r,
				minPrice.Mul(utils.ParseDec("1.01")),
...
func SimulateMsgWithdraw(
	...
	loop:
		...
		if skip {
			return simtypes.NoOpMsg(
				types.ModuleName,
				types.TypeMsgWithdraw,
				"no account to withdraw from pool",
			), nil, nil
		}

		poolCoin := sdk.NewCoin(
			pool.PoolCoinDenom,
			utils.RandomInt(r, sdk.OneInt(), spendable.AmountOf(pool.PoolCoinDenom)),
		)
...
func SimulateMsgLimitOrder(
	...
	return func(
		...
		if skip {
			return simtypes.NoOpMsg(
				types.ModuleName,
				types.TypeMsgLimitOrder,
				"no account to make a limit order",
			), nil, nil
		}
		...
		price := amm.PriceToDownTick(
			utils.RandomDec(r, minPrice, maxPrice),
			int(params.TickPrecision),
		)

		minAmt := sdk.MaxInt(
			amm.MinCoinAmount,
			sdk.NewDecFromInt(amm.MinCoinAmount).QuoRoundUp(price).Ceil().TruncateInt(),
		)
...
func SimulateMsgMarketOrder(
	...
	return func(
		...
		if skip {
			return simtypes.NoOpMsg(
				types.ModuleName,
				types.TypeMsgMarketOrder,
				"no account to make a market order",
			), nil, nil
		}

		minPrice, maxPrice := types.PriceLimits(
			*pair.LastPrice,
			k.GetMaxPriceLimitRatio(ctx),
			int(k.GetTickPrecision(ctx)),
		)

		minAmt := sdk.MaxInt(
			amm.MinCoinAmount,
			sdk.NewDecFromInt(amm.MinCoinAmount).QuoRoundUp(minPrice).Ceil().TruncateInt(),
		)
		amt := utils.RandomInt(r, minAmt, minAmt.MulRaw(100))
...
func SimulateMsgMMOrder(
	...
	return func(
		...
		if skip {
			return simtypes.NoOpMsg(
				types.ModuleName,
				types.TypeMsgMMOrder,
				"no account to make a market making order",
			), nil, nil
		}
...
func SimulateMsgCancelOrder(
	...
	return func(
		...
		for _, simAccount = range accs {
			...
			found := false
			_ = k.IterateOrdersByOrderer(
				ctx,
				simAccount.Address,
				func(order types.Order) (stop bool, err error) {
		...
		if skip {
			return simtypes.NoOpMsg(
				types.ModuleName,
				types.TypeMsgCancelOrder,
				"no account to cancel an order",
			), nil, nil
		}
...
func findPairToCreateRangedPool(
	r *rand.Rand,
	k keeper.Keeper,
	ctx sdk.Context,
	spendable sdk.Coins,
...
func findPairToMakeLimitOrder(
	r *rand.Rand,
	k keeper.Keeper,
	ctx sdk.Context,
	spendable sdk.Coins,
) (types.Pair, types.Pool, types.OrderDirection, bool) {
	var pairs []types.Pair
	_ = k.IterateAllPairs(ctx, func(pair types.Pair) (stop bool, err error) {
		pairs = append(pairs, pair)
		return false, nil
...
func findPairToMakeMarketOrder(
	r *rand.Rand,
	k keeper.Keeper,
	ctx sdk.Context,
	spendable sdk.Coins,
) (types.Pair, types.OrderDirection, bool) {
	var pairs []types.Pair
	_ = k.IterateAllPairs(ctx, func(pair types.Pair) (stop bool, err error) {
		pairs = append(pairs, pair)
		return false, nil
...
func minMaxPrice(k keeper.Keeper, ctx sdk.Context, lastPrice sdk.Dec) (sdk.Dec, sdk.Dec) {
	...
	maxPrice := amm.PriceToDownTick(
		lastPrice.Mul(sdk.OneDec().Add(params.MaxPriceLimitRatio)),
		tickPrec,
	)
	minPrice := amm.PriceToUpTick(
		lastPrice.Mul(sdk.OneDec().Sub(params.MaxPriceLimitRatio)),
		tickPrec,
	)
