...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			inactiveStr, _ := cmd.Flags().GetString(FlagInactive)
			if inactiveStr != "" {
				if _, err := strconv.ParseBool(inactiveStr); err != nil {
					return fmt.Errorf("parse Inactive flag: %w", err)
				}
			}
