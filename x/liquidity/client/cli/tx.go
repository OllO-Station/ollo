...
func NewCreateIntelligentPoolCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create-intelligent-pool [pair-id] [deposit-coins] [min-price] [max-price] [initial-price]",
		Args:  cobra.ExactArgs(5),
		Short: "Create a intelligent liquidity pool",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Create an intelligent liquidity pool with coins.

Example:
$ %s tx %s create-intelligent-pool 1 1000000000uatom,10000000000stake 0.001 100 1.0 --from mykey
$ %s tx %s create-intelligent-pool 1 1000000000uatom,10000000000stake 0.9 10000 1.0 --from mykey
$ %s tx %s create-intelligent-pool 1 1000000000uatom,10000000000stake 1.3 2.5 1.5 --from mykey
`,
				version.AppName, types.ModuleName,
				version.AppName, types.ModuleName,
				version.AppName, types.ModuleName,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgCreateIntelligentPool(
