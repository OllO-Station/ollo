...
func TestMakeOrderBookResponse(t *testing.T) {
	...
	resp := types.MakeOrderBookPairResponse(
		1,
		ov,
		lowestPrice,
		highestPrice,
...
func makeOrderBookPairResponse(
	numOrders, numPools, numTicks, tickPrec int,
) types.OrderBookPairResponse {
	...
	for i := 0; i < numPools; i++ {
		...
		ob.AddOrder(
			amm.PoolOrders(pool, amm.DefaultOrderer, lowestPrice, highestPrice, tickPrec)...)
	}
