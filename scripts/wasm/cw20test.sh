...
ollod keys add demo --keyring-backend test
VAL=$(ollod keys show -a validator --keyring-backend test)
DEMO=$(ollod keys show -a demo --keyring-backend test)
...
ollod tx wasm instantiate $CODE "$INIT" --label "First Coin" --no-admin --from validator \
...
ollod query wasm contract-state smart $CONTRACT "$QUERY"
echo "Demo Balance:"
ollod query wasm contract-state smart $CONTRACT "$QUERY_DEMO"
...
ollod tx wasm execute $CONTRACT "$TRANSFER" --from validator --keyring-backend test \
